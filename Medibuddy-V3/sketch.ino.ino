...
// ============================================================
bool userIsEditing() {
  return currentMode!=CLOCK_HOME && currentMode!=ALARM_ACTIVE && currentMode!=VIEW_SUMMARY;
}
...
void syncBoxesFromFirebase() {
  ...
  if(!Firebase.RTDB.getJSON(&fbdo,"boxes")) {
    Serial.println("Boot sync: no Firebase data, pushing local");
    pushBoxesToFirebase(); return;
  }
  FirebaseJson& json=fbdo.jsonObject();
  FirebaseJsonData d;
  if(!json.get(d,"/0/freq")) { pushBoxesToFirebase(); return; }
  bool changed=false;
  for(int i=0;i<itemCount;i++) {
    String base="/"+String(i)+"/";
    long st=T_7AM; int freq=0,intv=0,dose=1;
    if(json.get(d,base+"startSecs")) st  =d.intValue;
    if(json.get(d,base+"freq"))      freq=d.intValue;
    if(json.get(d,base+"interval"))  intv=d.intValue;
    if(json.get(d,base+"dose"))      dose=d.intValue;
    if(boxStartTimes[i]!=st||boxFrequencies[i]!=freq||boxIntervals[i]!=intv||boxPillDose[i]!=dose) {
      boxStartTimes[i]=st; boxFrequencies[i]=freq; boxIntervals[i]=intv; boxPillDose[i]=dose;
      changed=true;
    }
  }
  if(changed) saveToLittleFS();
  Serial.println(changed?"Boot sync: applied":"Boot sync: no changes");
}
...
// Old FirebaseJson version kept as stub — no longer used
void applyBoxDataFromJson(FirebaseJson& json) { }
...
void pollFirebase() {
  ...
  if(Firebase.RTDB.getJSON(&fbdo,"boxes")) {
    ...
    if(payload.length() > 10) {
      // Print first 300 chars of raw payload so we can see exact structure
      Serial.print("RAW: "); Serial.println(payload.substring(0, 300));
...
void loop() {
  ...
  if(wifiOK&&fbReady&&millis()-lastPoll>3000UL) {
    lastPoll=millis();
    pollFirebase();
  }
}
